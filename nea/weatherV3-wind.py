...
def tideIndex(data, datetime):
    idx=0
    for i,tdata in enumerate(data):
        if datetime == tdata[0]:
            #alread in the data, so ignore
            return True
        #update the idx if the datetime is greater
        if datetime > tdata[0]:
            idx=i
    return idx
    
def mergeTide(file, tide):
    ''' merge newly download tide data into file'''
    #empty tide data
    if len(tide) ==0:
        return
    prev_data=[]
    ...
    if os.path.exists(file):
        #read data into prev_data
        contents = open(file,'r').readlines()
        #skip first record
        for i in xrange(1,len(contents)):
            line =contents[i].split(',')
            if len(line) == 3:
                tide_ts = datetime.strptime(line[0],'%d/%m/%Y %H:%M:%S')
                tide_height = line[1].strip()
                tide_predict = line[2].strip()
                prev_data.append((tide_ts,tide_height,tide_predict))
        print "prev got ",len(prev_data)       
        #compare
        if len(prev_data) == len(tide) or len(prev_data) > len(tide):
            #same number of items, so ignore
            print "same or greater amount of data, ignore"
            return 
        # check prev_data got any data
        if len(prev_data) > 0:            
            #not same, have to merge
            for tide_data in tide:
                insert_idx = tideIndex(prev_data, tide_data[0])
                if insert_idx == True:
                    pass
                else:
                    prev_data.insert(insert_idx+1, tide_data)
                
    #write/overwrite data into file
    if len(prev_data) == 0:
        prev_data = tide
    with open(file, 'wb') as csvfile:
        spamwriter = csv.writer(csvfile, delimiter=',',
                quotechar='|', quoting=csv.QUOTE_MINIMAL)
        spamwriter.writerow(["Date Time(SGT)","Speed (km/h)","Dir"])                    
        for node in prev_data:
            spamwriter.writerow([node[0].strftime("%d/%m/%Y %H:%M:%S"), node[1], node[2]])

def getDetails(stationId, hrType=24):
    url = "http://www.weather.gov.sg/wp-content/themes/wiptheme/page-functions/functions-ajax-wind-chart.php"
    values= {
    ...
    winds = json.loads(the_page)
    ts = datetime.now() - timedelta(hours=hrType)
    base_date = date(ts.year, ts.month, ts.day)
    file="%s_wind_%s.csv" %(stationId,base_date.strftime("%Y-%m-%d"))
    '''
    [{"windSpeedKpr":"4.3000","time":"5 am","windDirection":"22"},{"windSpeedKpr":"4.3000","time":"5:01 am","windDirection":"22"}]
    '''
    winddata=[]
    for wind in winds:
        time=wind['time'] #need to re-arrange
        if "am" not in time and "pm" not in time:
            #update basedate, i.e. "2 Oct", how about year lap?
            base_ts = datetime.strptime(time, "%d %b")
            if "1 Jan" == time:
                base_date = date(base_date.year +1, base_ts.month, base_ts.day)
            else:
                base_date = date(base_date.year, base_ts.month, base_ts.day)
            #write file over
            mergeTide(file, winddata)
            #create new file
            winddata=[]
            file="%s_wind_%s.csv" %(stationId,base_date.strftime("%Y-%m-%d"))
            windtime = datetime(base_date.year, base_date.month, base_date.day, 0, 0, 0)
        else:
            #case "5 am"
            if ":" not in time:
                hour = time.split(" ")[0]
                ampm = time.split(" ")[1]
                time="%s:00 %s" % (hour, ampm)
            base_ts = datetime.strptime(time, "%I:%M %p")
            windtime = datetime(base_date.year, base_date.month, base_date.day, base_ts.hour, base_ts.minute, 0)
        if "windSpeedKpr" in wind and "windDirection" in wind:
            windspeed=  wind['windSpeedKpr']
            winddir= wind['windDirection']
            winddata.append((windtime, windspeed, winddir))
    mergeTide(file, winddata)
    #write to tfile
...
'''
import sys
url='http://www.weather.gov.sg/weather-currentobservations-wind'
...
obvspan=soup.find('span',class_="date-obs")
print obvspan.string    
m=re.search(r"Observations at (.*)",obvspan.string)
if m is not None:
    print m.group(1)
    dt=datetime.strptime(m.group(1),"%I.%M %p %d %b")
    file="%s-%s-wind.csv" %(datetime.now().strftime("%Y"), dt.strftime("%m-%d_%H%M"))
    fcsv=open(file,'w')
    fcsv.write("Station, Value(km/h)\n")
    spans=soup.find_all('span',class_="sgr")
    for span in spans:
        stdid = span['id']
        stdval = span.contents[0].string    
        fcsv.write("%s, %s\n" %(stdid, stdval))
    fcsv.close()
