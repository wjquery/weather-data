...
'''
==== RAIN FALL DATA API ======
URL: http://www.weather.gov.sg/wp-content/themes/wiptheme/page-functions/functions-weather-current-observations-rainfall-ajax.php
HTTP POST:
    tableName: 	wip_c_rain_data_30min | wip_c_rain_data_60min | wip_c_rain_data_120min | wip_c_rain_data_midnight 
    type: html
Response: html script in following format
    <span id="S07" name="points" class="sgr" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="
      <ul class='list-unstyled'>
        <li>Location: 
          <strong>Macritchie Reservoir</strong>
        </li>
      </ul>" data-original-title="" title="" onmousemove="showinfoByClickSpan()"  onclick="showByClickSpan('S07')" style=display:none>0
    </span>
    Need to extract the span id and value(text)
    
==== RAIN FALL IMAGE API ======
URL: 

- http://www.weather.gov.sg/files/isohyet/isohyet.png?time=2015-07-10 14:44:59
- http://www.weather.gov.sg/files/isohyet/isohyet60min.png?time=2015-07-10 14:44:59
- http://www.weather.gov.sg/files/isohyet/isohyet120min.png?time=2015-07-10 14:44:59
- http://www.weather.gov.sg/files/isohyet/isohyetmidnight.png?time=2015-07-10 14:44:59
...
'''
tableNames=['wip_c_rain_data_30min','wip_c_rain_data_60min','wip_c_rain_data_120min','wip_c_rain_data_midnight']
...
response.close()
m=re.search(r"http://www.weather.gov.sg/files/isohyet/isohyet.png\?time=(.*)\'",html_doc,flags=re.M)
if m is not None:
    obv_time = m.group(1)
else:
    print "Error"
...
#2. get 30min 60min, 120min and since midnight data
for tlb in tableNames:
    file="%s-%s.csv" %(obv_time.replace(':','').replace(' ','_'), tlb)
    fcsv=open(file,'w')
    fcsv.write("Station, Value \n")
    data_url='http://www.weather.gov.sg/wp-content/themes/wiptheme/page-functions/functions-weather-current-observations-rainfall-ajax.php'
    #img_url=
    values={'tableName':tlb, 'type':'html'}
